...
async def root():
    ...
    Sends browsers to the study console when it is bundled, and reports
    API information otherwise.
    """
...
# Serve the study console from the API itself, so the browser calls a
# same-origin /api and no file:// CORS dance is needed. Mounted last so
# it can never shadow an API route.
FRONTEND_DIR = Path(__file__).resolve().parent.parent / "frontend"

if FRONTEND_DIR.is_dir():
    app.mount("/app", StaticFiles(directory=FRONTEND_DIR, html=True), name="console")
    logger.info(f"Study console served from {FRONTEND_DIR} at /app")
else:
