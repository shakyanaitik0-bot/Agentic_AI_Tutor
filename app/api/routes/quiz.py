...
def generate_quiz(
    ...
    if not active_session:
        # Create new session
        active_session = DBSession(student_id=student.id, is_active=True, session_type="quiz")
    ...
    try:
        # Create quiz agent
        quiz_agent = QuizGeneratorAgent(student=student, session=active_session, db=db)

        # Generate quiz
        user_message = f"Generate a quiz on {request.topic}"
        kwargs = {"topic": request.topic, "num_questions": request.num_questions}
...
def submit_quiz(
    ...
    try:
        ...
        logger.info(
            f"Quiz graded: {response.accuracy:.1f}% ({response.correct_answers}/{response.total_questions})"
        )
